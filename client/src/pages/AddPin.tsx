...
import { ArrowLeft, Footprints, Bus, Car, Sunrise, Sunset, Plus, X, MapPin, ChevronRight } from 'lucide-react'
...
export default function AddPin() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // include any URL still typed in the input but not yet added
      const pendingUrl = videoInput.trim()
      const finalVideos = pendingUrl && !videos.includes(pendingUrl)
        ? [...videos, pendingUrl]
        : videos
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-4 flex-1 overflow-y-auto pb-8">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <button
              ...
              className={`flex items-center gap-3 h-14 px-4 rounded-2xl border transition-colors ${
                pickedLocation
                  ? 'bg-primary/5 border-primary/40 text-foreground'
                  : 'bg-secondary border-border text-muted-foreground'
              }`}
            >
              <MapPin className={`size-5 shrink-0 ${pickedLocation ? 'text-primary' : 'text-muted-foreground'}`} />
              <span className="flex-1 text-sm text-left font-medium truncate">
                {pickedLocation
